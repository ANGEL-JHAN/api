...
let apiKeys = [
  {
    ...
    plan: "admin",
    uso: 0,
    limite: Infinity, // Admin infinito
  },
];
...
function validarKey(key, res) {
  ...
  // revisar límite solo si no es admin
  if (keyData.plan !== "admin" && keyData.uso >= keyData.limite) {
...
app.post("/api/ia", async (req, res) => {
  ...
  try {
    ...
    // actualizar uso solo si no es admin
    if (keyData.plan !== "admin") keyData.uso++;
    guardarKeys();
    ...
    try {
      ...
      console.error("❌ DB externa:", err.message);
    }
